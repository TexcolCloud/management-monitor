...
class FeishuOutboundBotTest(unittest.TestCase):
    ...
    def test_send_work_order_uses_outbound_api_without_callback(self) -> None:
        ...
        self.assertEqual(
            json.loads(payload["content"])["text"],
            "【工单编号】 CODE-1\n【所属班组】 -\n【工单主题】 New order\n【创建时间】 -",
        )
    ...
    def test_send_file_uploads_explicit_filename_and_preserves_extension(self) -> None:
        cases = (
            ("daily-management-export.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"),
            ("training-material.pdf", "application/pdf"),
            ("training-material.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
            ("training-slides.pptx", "application/vnd.openxmlformats-officedocument.presentationml.presentation"),
        )
        for filename, mime_type in cases:
            with self.subTest(filename=filename), tempfile.TemporaryDirectory() as temp_dir:
                ...
                bot._tenant_access_token = "token"
                bot._token_expires_at = 2_000_000_000
                export = Path(temp_dir) / filename
                export.write_bytes(b"file-content")
                with patch("common.feishu_app_bot.time.time", return_value=1_700_000_000), patch(
                    "common.feishu_app_bot.urllib.request.urlopen",
                    side_effect=[
                ...
                self.assertIn(b'filename="' + encoded_name + b'"', upload_request.data)
                self.assertIn(f"Content-Type: {mime_type}".encode("ascii"), upload_request.data)
