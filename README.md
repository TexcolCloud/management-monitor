...
同一个已登录浏览器会话。初始化不生成本地 Excel；仅在飞书收到已授权用户的导出请求后，
才按用户选择的日期范围临时生成并私发 Excel。
...
详情、附件清单、附件文件和 SHA-256 后发布快照，入库并进入常驻监听，不会创建本地 Excel。
已授权用户在飞书发送导出命令并提交日期后，服务才临时生成 Excel 并私发。使用 `Ctrl+C`
平稳停止。
